...
import { ReferralCodeModel } from '../../models/ReferralCode.ts'
...
describe('ReferralCleanupService', () => {
  ...
  describe('cleanupExpiredCodes', () => {
    ...
    it('should deactivate expired referral codes', async () => {
      ...
      const inactiveCount = await ReferralCodeModel.countDocuments({
        isActive: false
      })
  ...
  describe('extendExpiration', () => {
    ...
    it('should extend expiration for existing referral code', async () => {
      const initialCode = await ReferralCodeModel.findOne({
        walletAddress: TEST_WALLETS.referrer
      })
      ...
      const updatedCode = await ReferralCodeModel.findOne({
        walletAddress: TEST_WALLETS.referrer
      })
      const newExpiration = updatedCode?.expiresAt

      expect(result).toBe(true)
      expect(newExpiration?.getTime()).toBeGreaterThan(originalExpiration?.getTime() || 0)
      const diffDays =
        ((newExpiration?.getTime() || 0) - (originalExpiration?.getTime() || 0)) /
        (1000 * 3600 * 24)
    ...
    it('should return codes sorted by expiration date', async () => {
      ...
      for (let i = 0; i < expiringCodes.length - 1; i++) {
        expect(expiringCodes[i]?.expiresAt?.getTime()).toBeLessThanOrEqual(
          expiringCodes[i + 1]?.expiresAt?.getTime() || 0
        )
      }
    })
