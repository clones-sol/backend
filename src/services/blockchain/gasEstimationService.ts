...
const _DEFAULT_TOKEN_DECIMALS = 6
...
class GasEstimationService {
  ...
  async estimateBatchClaimGas(claims: ClaimData[], fromAddress: string): Promise<GasEstimate> {
    ...
    try {
      ...
      const gasLimit = await claimRouter.claimAll.estimateGas(claims, {
        from: fromAddress
      })
  ...
  async analyzeClaimGasCost(
    ...
    const gasCostVsReward =
      netRewardUsd > 0 ? (gasEstimate.totalGasCostUsd / netRewardUsd) * 100 : Infinity
    ...
    if (gasCostVsReward > VERY_HIGH_GAS_COST_THRESHOLD) {
      recommendation =
        'Gas cost is very high compared to reward. Consider waiting for lower gas prices or accumulating more rewards.'
  ...
  async getGasOptimizationAdvice(): Promise<{
    ...
    if (gasPriceGwei < GAS_PRICE_LEVELS.LOW) {
      ...
      recommendation =
        'Gas prices are reasonable. Good time to claim if you have significant rewards.'
      ...
      suggestedWaitTime =
        'Check back during off-peak hours (typically late night/early morning UTC).'
    }
