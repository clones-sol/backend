...
describe('Forge Gas API', () => {
  ...
  describe('POST /api/v1/forge/gas/estimate', () => {
    it('should estimate gas for a batch claim successfully', async () => {
      ...
      expect(mockGasEstimationService.estimateBatchClaimGas).toHaveBeenCalledWith(
        dummyClaims,
        TEST_WALLETS.fromAddress
      )
    })
  ...
  describe('POST /api/v1/forge/gas/analyze', () => {
    it('should analyze gas cost successfully', async () => {
      ...
      expect(mockGasEstimationService.analyzeClaimGasCost).toHaveBeenCalledWith(
        dummyClaims,
        TEST_WALLETS.fromAddress,
        1
      )
    ...
    it('should allow overriding tokenPriceUsd', async () => {
      ...
      await supertest(app)
        ...
        .send({
          claims: dummyClaims,
          fromAddress: TEST_WALLETS.fromAddress,
          tokenPriceUsd: 0.5
        })
        .expect(200)
  ...
  describe('POST /api/v1/forge/gas/optimize', () => {
    it('should optimize batch size successfully', async () => {
      const mockOptimization = {
        optimizedBatches: [dummyClaims],
        recommendation: 'One batch is fine'
      }
      ...
      expect(mockGasEstimationService.optimizeBatchSize).toHaveBeenCalledWith(
        dummyClaims,
        TEST_WALLETS.fromAddress,
        50
      )
  ...
  describe('GET /api/v1/forge/gas/advice', () => {
    it('should get gas advice successfully', async () => {
      ...
      const response = await supertest(app).get('/api/v1/forge/gas/advice').expect(200)
