...
vi.mock('ethers', () => ({
  ethers: {
    ...
    parseUnits: vi.fn((value: string, unit: number) => {
      ...
      return BigInt(Math.floor(parseFloat(value) * Math.pow(10, unit)));
    }),
    formatUnits: vi.fn((value: bigint, unit: number) => {
      return (Number(value) / Math.pow(10, unit)).toString();
    })
  }
}));
...
describe('TokenCache', () => {
  ...
  describe('getTokenMetadata', () => {
    ...
    it('should handle different token addresses separately', async () => {
      ...
      mockContract.decimals.mockResolvedValueOnce(expectedMetadata.decimals)
                          .mockResolvedValueOnce(token2Metadata.decimals);
      mockContract.symbol.mockResolvedValueOnce(expectedMetadata.symbol)
                         .mockResolvedValueOnce(token2Metadata.symbol);
      mockContract.name.mockResolvedValueOnce(expectedMetadata.name)
    ...
    it('should handle blockchain errors gracefully', async () => {
      ...
      await expect(tokenCache.getTokenMetadata(testTokenAddress, mockProvider))
  ...
  describe('validateAndParseAmountWithMetadata', () => {
    ...
    it('should handle invalid amounts with proper error messages', async () => {
      await expect(tokenCache.validateAndParseAmountWithMetadata('invalid', testTokenAddress, mockProvider))
  ...
  describe('Race condition handling', () => {
    it('should handle concurrent requests for same token', async () => {
      ...
      results.forEach(result => expect(result).toEqual(expectedMetadata));
