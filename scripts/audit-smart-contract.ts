...
async function performSecurityAnalysis(connection: Connection, programPubkey: PublicKey, result: AuditResult) {
  const passed: string[] = [];
  ...
  try {
    // Try to get platform stats to verify access control
    const rewardPoolService = new RewardPoolService(
      connection,
      PROGRAM_ID,
      null as any, // We don't need the keypair for read operations
    ...
    if (stats) {
      ...
      warnings.push('⚠️ Access control: Unable to verify platform authority');
    }
  } catch (error) {
    warnings.push(`⚠️ Access control: ${error.message}`);
  }
...
async function performCodeQualityAnalysis(result: AuditResult) {
...
async function performBlockchainAnalysis(connection: Connection, programPubkey: PublicKey, result: AuditResult) {
  try {
...
function determineStatus(overallScore: number): string {
...
function generateRecommendations(result: AuditResult) {
...
function displayAuditResults(result: AuditResult) {
  console.log('\n' + '='.repeat(60));
...
async function exportAuditReport(result: AuditResult) {
